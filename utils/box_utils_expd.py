...
def bbox_overlaps_giou(bboxes1, bboxes2):
    ...
    rious = torch.zeros((rows, cols))
    if rows * cols == 0:
        return rious
    exchange = False
    if bboxes1.shape[0] > bboxes2.shape[0]:
        bboxes1, bboxes2 = bboxes2, bboxes1
        rious = torch.zeros((cols, rows))
        exchange = True
    ...
    rious = inter_area / union - torch.exp((inter_diag) / outer_diag)+1
    rious = torch.clamp(rious,min=-1.718,max = 1.0)
    #print(rious)
    if exchange:
        rious = rious.T
    return rious
...
def nms(boxes, scores, overlap=0.1, top_k=200):
    ...
    while idx.numel() > 0:
        ...
        center_x2 = (xx1 + xx2) / 2
        center_y2 = (yy2 + yy2) / 2
        u = (center_x1 - center_x2) ** 2 + (center_y1 - center_y2) ** 2
        ...
        #IoU = inter/union - torch.pow(d , 1.1) # store result in iou
        IoU = inter/union
